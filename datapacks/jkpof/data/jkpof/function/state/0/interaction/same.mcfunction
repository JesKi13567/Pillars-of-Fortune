...
execute as @s[tag=jkpof_ctrl_touch_change] run function jkpof:state/0/interaction/ctrl/switcher {id: 'touch_change', item: 'rabbit_foot'}
execute as @s[tag=jkpof_ctrl_touch_change] if score #ctrl_touch_change jkpof.int matches 1 run tellraw @a ["", {storage: "jk:pof", interpret: true, nbt: "txt.POF", color: "yellow"}, {storage: "jk:pof", interpret: true, nbt: "txt.lobby.const.settings.special_rules.touch_change.name", color: "green"}, {storage: "jk:pof", interpret: true, nbt: "txt.char.colon"}, {storage: "jk:pof", interpret: true, nbt: "txt.lobby.const.settings.special_rules.touch_change.lore"}]
