data modify storage jk:pof data.stats set from storage jk:pof txt.lobby.const.stats
data modify storage jk:pof txt.map.title.15 set from storage jk:pof txt.map.global.mix
...
data modify entity @e[type=text_display, tag=jkpof_display_ctrl_player_list, limit=1] text set value [{storage: "jk:pof", interpret: true, nbt: "txt.lobby.const.settings.others.player_list", color: "green"}]
