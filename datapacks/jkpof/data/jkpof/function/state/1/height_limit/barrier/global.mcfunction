execute if score #ctrl_upside_down jkpof.int matches 0 as @s[tag=max, scores={jkpof.int=1..}] run function jkpof:state/1/height_limit/barrier/tp {sb: '-'}
execute if score #ctrl_upside_down jkpof.int matches 1 as @s[tag=min, scores={jkpof.int=1..}] run function jkpof:state/1/height_limit/barrier/tp {sb: ''}
