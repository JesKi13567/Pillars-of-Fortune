...
# 种类
execute if score #supply_real jkpof.int matches 4.. as @a[scores={jkpof.state=2}, gamemode=survival] store result score @s jkpof.item.order run random value 1..14
...
# 道具
execute as @a[scores={jkpof.state=2, jkpof.item.order=2..14}, gamemode=survival] run loot give @s loot jkpof:item/prop/common
...
# 其他
execute as @a[scores={jkpof.state=2}, gamemode=survival] unless score @s jkpof.item.order matches 1..14 run loot give @s loot jkpof:item/main
