...
# 分数
execute if score #is_prop jkpof.int matches 0 store result score #item_order jkpof.int run random value 1..1196
...
# 真正
execute if score #event_prop_supply.count.enchanted_book jkpof.int matches 1.. run function jkpof:state/1/give/extra {A: 'r', S: 'a[scores={jkpof.state=2}]', I: 'enchanted_book'}
execute if score #event_prop_supply.count.potion jkpof.int matches 1.. run function jkpof:state/1/give/extra {A: 'r', S: 'a[scores={jkpof.state=2}]', I: 'potion'}
function jkpof:state/1/give/real {A: 'r[', S: 'a[scores={jkpof.state=2}]'}
