...
$execute if score #respawn.state jkpof.int matches 0 run tellraw @a ["", {storage: "jk:pof", interpret: true, nbt: "txt.POF.show", color: "yellow"}, "$(id)", {storage: "jk:pof", interpret: true, nbt: "txt.lobby.item.player_head.tellraw.fail", color: "red"}]
