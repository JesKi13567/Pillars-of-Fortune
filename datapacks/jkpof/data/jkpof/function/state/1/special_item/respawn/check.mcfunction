...
$execute if score #respawn.state jkpof.int matches 0 run tellraw @a ["", {storage: "jk:pof", interpret: true, nbt: "txt.POF", color: "yellow"}, "$(id)", {storage: "jk:pof", interpret: true, nbt: "txt.lobby.item.respawn_device.tellraw.fail", color: "red"}]
