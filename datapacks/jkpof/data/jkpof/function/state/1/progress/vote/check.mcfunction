...
execute store result score @e[type=marker, tag=jkpof_votes, tag=-1, limit=1] jkpof.int if entity @a[scores={jkpof.vote=7}, gamemode=spectator]
...
execute if entity @e[type=marker, tag=jkpof_votes_max, tag=-1] run tellraw @a[gamemode=spectator] ["[", {storage: "jk:pof", nbt: "txt.event.border.name"}, "]", {text: " -> ", color: "gray"}, {selector: "@a[scores={jkpof.vote=7}, gamemode=spectator]"}]
