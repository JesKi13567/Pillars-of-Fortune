...
execute if score #event_plus_3 jkpof.int matches 1 run tellraw @a[tag=!jkpof_lightning_player] [{storage: "jk:pof", interpret: true, nbt: "txt.POF", color: "yellow"}, {storage: "jk:pof", interpret: true, nbt: "txt.event.plus.lightning.name", color: "aqua"}, {storage: "jk:pof", interpret: true, nbt: "txt.event.global.stage_ended"}]
