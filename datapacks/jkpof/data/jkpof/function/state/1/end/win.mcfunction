...
execute unless score #event_betrayal_hour.status jkpof.int matches 1 run scoreboard players operation #win jkpof.team = @a[limit=1, tag=jkpof_winner] jkpof.team
execute unless score #event_betrayal_hour.status jkpof.int matches 1 as @a[scores={jkpof.state=2..3}] if score @s jkpof.team = #win jkpof.team run tag @s add jkpof_winner
execute if score #event_betrayal_hour.status jkpof.int matches 1 if entity @a[scores={jkpof.state=2..3}, limit=1, tag=jkpof_winner, team=jkpof.allies] run tag @a[team=jkpof.allies] add jkpof_winner
