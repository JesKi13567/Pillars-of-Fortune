...
|事件列表|描述|
|-|-|
|清理垃圾（系统事件）|系统每 2 分钟清理一次掉落物。若有玩家死亡，下次清理时间增加 30 秒。|
...
|道具补给|接下来的 3 次物品给予将随机触发以下情况之一：1. 均有 60% 概率为道具。2. 额外附赠随机附魔书。3. 额外附赠随机药水。|
...
|事件列表|說明|
|-|-|
|清理垃圾（系統事件）|系統每 2 分鐘清理一次掉落物。若有玩家死亡，下次清理時間增加 30 秒。|
...
|道具補給|接下來的 3 次給予物品將隨機觸發以下情況之一：1. 均有 60% 概率為道具。2. 額外附贈隨機附魔書。3. 額外附贈隨機藥水。|
...
|鞘翅|穿著任意胸甲，手持鞘翅並潛行，胸甲激活滑翔效果。若成功激活，將獲得隨機的煙火。|
...
|命令列表|描述|
|-|-|
|`/function jkpof:init`|重置/初始化整個地圖。|
|`/function jkpof:test/_start`|以測試模式開啟遊戲（單人亦可），但無法正常結束。|
|`/function jkpof:test/_stop`|強制結束當前遊戲。|
|`/function jkpof:test/give`|所有人清空揹包，獲得一堆隨機道具，但會導致物品給予延後。|
|`/function jkpof:test/next_event`|直接觸發當前事件。|
...
|Events|Description|
|-|-|
|Clear trash (System Event)|The system clears dropped items every 2m. If a player dies, the next cleanup time will be delayed by 30s.|
...
|Prop Supply|The next 3 items players receive will be one of the following:1. 60% chance of being props. 2. Plus a random Enchanted Book. 3. Plus a random Potion.|
